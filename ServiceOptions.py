...
class ServiceOptions:
	...
	def __init__(self):
		...
		# define running machine
		# self.execMachine = config.get('Machine', 'Name') # ExecMachine.PC_MintCinnamon

		# self.FILENAME_WITHOUT_EXT = f'{self.GLOBAL_NOW.strftime("%Y.%m.%d %H%M%S")}_openweather'

		# self.db_Conn = DBConnection(self.config.dbConnParameter)
		# self.logMe = LogMe(self.config.LOG_DIR, self.FILENAME_WITHOUT_EXT)
	...
	def create_backup_dir_if_not_exists(self):
		...
		try:
			ServiceOptions.create_dir_if_not_exists(self.backup_directory)
	...
	def create_log_dir_if_not_exists(self):
		...
		if self.log_on:
			try:
				ServiceOptions.create_dir_if_not_exists(self.log_directory)
	...
	def create_csv_dir_if_not_exists(self):
		...
		if self.csv_on:
			try:
				ServiceOptions.create_dir_if_not_exists(self.csv_directory)
	...
	def create_raw_data_dir_if_not_exists(self):
		...
		try:
			ServiceOptions.create_dir_if_not_exists(self.rawdata_directory)
